 ...
 */
"use strict";var isDate=_dereq_("is-date-object"),padStart=_dereq_("string.prototype.padstart"),map=_dereq_("array-map-x");module.exports=function toISOString(date){if(!1===isDate(date))throw new TypeError("toISOString called on incompatible receiver.");if(!1===isFinite(date)||!1===isFinite(date.getTime()))throw new RangeError("toISOString called on non-finite value.");var year=date.getUTCFullYear(),month=date.getUTCMonth();year+=Math.floor(month/12);var sign,parts=[1+(month=(month%12+12)%12),date.getUTCDate(),date.getUTCHours(),date.getUTCMinutes(),date.getUTCSeconds()];year=(sign=year<0?"-":year>9999?"+":"")+padStart(Math.abs(year),sign?6:4,"0");var result=map(parts,function _for(item){return padStart(item,2,"0")}),dateStr=year+"-"+result.slice(0,2).join("-"),msStr=padStart(date.getUTCMilliseconds(date),3,"0");return dateStr+"T"+result.slice(2).join(":")+"."+msStr+"Z"}},{"array-map-x":2,"is-date-object":26,"string.prototype.padstart":48}],2:[function(_dereq_,module,exports){/**
